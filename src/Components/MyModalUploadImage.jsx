...
const MyModalUploadImage = () => {
  ...
  const showModalImageUpload = useSelector((state) => state.ModalUploadImage.showModalImageUpload)
  ...
  const postUploadImage = () => {
    ...
    fetch(`https://striveschool-api.herokuapp.com/api/profile/${myProfile._id}/picture`, {
      method: 'POST',
      headers: {
  ...
  return (
    <>
      <Modal show={showModalImageUpload} onHide={handleClose} data-bs-theme="dark" className="text-light" size="lg">
        <Modal.Header closeButton>
          <Modal.Title>Foto profilo</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div>
            <div className="overflow-hidden mx-auto" style={{ width: '280px', height: '280px' }}>
              <img src={myProfile.image} alt=" profile" className=" rounded-circle w-100 h-100" />
            </div>
        ...
        <hr className="my-2" />
        <div className="d-flex align-items-center justify-content-between mx-3 pb-3" style={{ paddingInline: '16px' }}>
          <div className="d-flex gap-2">
            <Button variant="outline-light border border-0" className="d-flex flex-column align-items-center">
              <FaPen />
              <span>Modifica</span>
            </Button>
            <Button
        ...
        <Modal.Body>
          <div className="d-flex flex-column align-items-center justify-content-between">
            <div>
              <h4>
                {' '}
                <span className="fw-semibold">{myProfile.name}</span>, aiuta gli altri a riconoscerti!
              </h4>
            </div>
            <div className="overflow-hidden mt-5" style={{ width: '180px', height: '180px' }}>
              <img src={myProfile.image} alt=" profile" className=" rounded-circle w-100 h-100" />
            </div>
            ...
            <div>
              <p className="mt-3">
                {' '}
                Chiediamo agli utenti di LinkedIn di utilizzare le loro vere identità, quindi scatta o <br /> carica una
                tua foto. Poi ritagliala, applica dei filtri e perfezionala come vuoi.
              </p>
            </div>
          </div>
        </Modal.Body>
