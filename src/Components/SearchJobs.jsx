...
import { Alert, Button, Card, Col, Container, Nav, Row, Tab } from 'react-bootstrap'
import LoadingSearch from './LoadingSearch'
import { useDispatch, useSelector } from 'react-redux'
import { addSavedJob, getJob, removeSavedJob } from '../redux/actions'
...
const SearchJobs = () => {
  ...
  const companyFetch = async (url = URL) => {
    try {
      ...
      console.log(error)
      // <Alert>Nessun risultato trovato per questa ricerca</Alert>
      setIsError(true)
      setTimeout(() => {
        navigate('/genericjobs')
      }, 2000)
    }
  }
  ...
  return (
    <>
      {isError && (
        <Alert className="mx-auto" style={{ marginTop: 80, maxWidth: 1128 }} variant="info">
      ...
      {!isError && (
        <Container className="bg-white rounded customRow  " style={{ maxWidth: '1128px', marginTop: '65px' }}>
          {company ? (
            <Tab.Container className=" h-100" defaultActiveKey={selectedJob ? selectedJob._id : company.data[0]._id}>
              <Row className="h-100  ">
                            ...
                            <div>
                              ...
                              <div className="d-flex gap-2">
                                ...
                                <Button
                                  ...
                                  variant={`${!isSaved(compagnie._id) ? 'outline-primary' : 'success'}`}
                                  onClick={() => {
                                ...
                                >
                                  {`${!isSaved(compagnie._id) ? 'Salva' : 'Salvato'}`}
                                </Button>
                              </div>
                              ...
                              <div className="border p-2">
                                ...
                                <p style={{ fontSize: '0.7rem' }} className="text-secondary p-1 ms-1 ">
                                  Prova gratuita di 1 mese. Ti invieremo un promemoria 7 giorni prima della fine del
                                  periodo di prova.
                                </p>
                              </div>
                            </div>
